...
sub create {
	...
	my %elements;
	my $count = 0;
	foreach my $item (@{$args->{sequence}}) {
		$count++;
		my $ref = ref $item || '';
		if (
			   ($ref && $ref ne 'ConvoTreeEngine::Object::Element' && $ref ne 'ConvoTreeEngine::Object::Series')
			|| (!$ref && $item !~ m/^(series)[0-9]+\z/i)
		  ) {
			ConvoTreeEngine::Exception::Input->throw(
				error => 'Items in a series sequence must be a series or an element',
			)
		}

		if (!$ref) {
			if ($item =~ m/^series([0-9]+)\z/i) {
				my $id = $1;
				$ref = 'ConvoTreeEngine::Object::Series';
				$item = $ref->findOrDie({id => $id}, {shallow => 1});
			}
			else {
				$ref = 'ConvoTreeEngine::Object::Element';
				$item = $ref->findOrDie({id => $item}); 
			}
		}

		if ($ref eq 'ConvoTreeEngine::Object::Element') {
			$elements{$item->id} = $count;
			my $lists = $item->seriesLists;
			##### TODO: Figure out where to go from here
		}
		else (
			##### TODO: What happens if it's a series
		)
	}
}
